...
function RoadmapResult() {
  ...
  // Seed from navigation state; fall back to null so we can fetch by ID if needed
  const [roadmap, setRoadmap] = useState(state?.roadmap || null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  useEffect(() => {
    // If we already have roadmap data from navigation state, nothing to do
    if (roadmap) return;

    // If there's a roadmapId in state (but no full data), fetch it
    ...
    // No state at all (e.g. hard refresh) — send them back to the form
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <p className="text-gray-400 animate-pulse">Loading your roadmap…</p>
      </div>
    );
  }
  ...
  // ── No data (guards against the render before useEffect redirect fires) ────
            ...
            {roadmap.universities?.map((uni, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span
                    className={`text-sm px-3 py-1 rounded-full ${
                      uni.eligibility === "safe"
                        ? "bg-green-500/20 text-green-400"
                        : uni.eligibility === "match"
                ...
                <p className="text-gray-400 text-sm">
                  {uni.program} · {uni.duration} · {uni.location}
                </p>
            ...
            {roadmap.timeline?.map((item, i) => (
              ...
              >
                <span className="text-cyan-400 font-semibold w-20 shrink-0">
                  {item.monthsFromNow === 0
                    ? "Now"
                    : `${Math.abs(item.monthsFromNow)}mo before`}
                </span>
                <div>
        ...
        <section className="mb-10">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <p className="text-gray-400 text-sm mb-4">
              Processing time: {roadmap.visa?.processingTime}
            </p>
        ...
        <section className="mb-10">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <p className="text-gray-400 text-sm mb-3">
              {roadmap.postStudy?.workPermit?.duration}
            </p>
            <p className="font-semibold">{roadmap.postStudy?.prPathway?.name}</p>
            <p className="text-gray-400 text-sm">
              {roadmap.postStudy?.prPathway?.description}
            </p>
          </div>
        </section>
